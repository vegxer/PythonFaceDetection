...
class SettingsWindowCNN:
    ...
    def open_dialog(self, root):
        ...
        tkinter.Label(self.__settings, text="Choose CNN training file", font="Arial 11").place(x=5, y=0)

        self.__list = ttk.Combobox(self.__settings, value=("...", "Standard CNN file", "Choose .dat from computer..."),
                                   width=25)
        self.__list.bind("<<ComboboxSelected>>", self.__save_training_file)
        self.__list.current(0)
        self.__list.place(x=180, y=3)

        tkinter.Label(self.__settings, text="Input number of pooling layers (from 0 to 3)", font="Arial 11").place(x=5,
                                                                                                                   y=30)
        ...
        if self.__list.current() == 2:
            ...
            if os.path.exists("Training_files\\mmod_human_face_detector.dat"):
                ...
                messagebox.showerror("File not found",
                                     f"File \'{str(pathlib.Path.cwd())}\\Training_files\\mmod_human_face_detector.dat\' hasn't been found")
